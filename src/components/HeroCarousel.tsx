...
export const HeroCarousel: React.FC = () => {
  ...
  return <div className="relative w-full bg-gradient-to-br from-blue-900 via-blue-800 to-blue-600 pt-16">
      {/* Main Carousel */}
            ...
            {slides.map((slide, index) => <CarouselItem key={index}>
                ...
                backgroundImage: `url(${slide.image})`
                  ...
                  <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex space-x-2 z-30">
                    {slides.map((_, i) => <div key={i} className={`w-3 h-3 rounded-full ${i === index ? 'bg-white' : 'bg-white/50'}`} />)}
                  </div>
                </div>
              </CarouselItem>)}
          </CarouselContent>
      ...
      <div className="bg-white py-4 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-center items-center gap-2 md:gap-3 lg:gap-4 flex-wrap my-[50px]">
            {salesforceServices.map((service, index) => <div key={index} className="flex flex-col items-center group cursor-pointer">
